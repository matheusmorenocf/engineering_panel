...
export const ManagementModal = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  ...
  // ✅ Puxa os dados e a função de recarregar do hook
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setInputValue('');
        refresh(); // Atualiza o hook e o cache
      }
      ...
      <div className="bg-surface border border-border w-full max-w-md rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 space-y-6 flex-1 overflow-hidden flex flex-col">
          <div className="flex gap-2 p-1 bg-bg rounded-xl border border-border shrink-0">
            <button onClick={() => setMode('setor')} className={`flex-1 py-2 text-[10px] font-black uppercase rounded-lg transition-all ${mode === 'setor' ? 'bg-secondary text-white' : 'text-text-tertiary'}`}>Setores</button>
            <button onClick={() => setMode('tipo')} className={`flex-1 py-2 text-[10px] font-black uppercase rounded-lg transition-all ${mode === 'tipo' ? 'bg-secondary text-white' : 'text-text-tertiary'}`}>Tipos</button>
          </div>

          <div className="flex gap-2 shrink-0">
            <input type="text" placeholder={`NOVO ${mode.toUpperCase()}...`} className="flex-1 bg-bg border border-border rounded-xl px-4 py-3 text-[10px] font-bold uppercase outline-none focus:border-secondary text-text-primary" value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAdd()}/>
            <button onClick={handleAdd} disabled={!inputValue.trim()} className="p-3 bg-secondary text-white rounded-xl hover:bg-secondary-dark disabled:opacity-50 transition-colors shadow-sm"><Plus size={20} /></button>
          </div>
          ...
          <div className="space-y-2 overflow-y-auto custom-scrollbar pr-1 flex-1">
            {loading && currentItems.length === 0 ? (
              ...
              <div key={item.id} className="flex items-center justify-between p-3 pl-4 bg-bg/50 border border-border rounded-xl group hover:border-secondary/30 transition-all text-text-primary">
                ...
                <button onClick={() => handleDelete(item.id)} className="p-2 text-text-tertiary hover:text-white hover:bg-red-500 rounded-lg transition-all opacity-0 group-hover:opacity-100"><Trash2 size={14} /></button>
              </div>
            ))}
          </div>
        </div>
