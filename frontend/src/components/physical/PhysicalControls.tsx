...
export function PhysicalControls({
  search, setSearch, viewMode, setViewMode, isRefreshing, onRefresh, onOpenLocationMgr, showAdminActions
  ...
  return (
    <div className="flex gap-4 items-center bg-card p-4 rounded-xl border shadow-sm shrink-0">
      <div className="relative flex-1">
        ...
        <Input 
          placeholder="Buscar por Produto, NF ou ID..." 
      ...
      <div className="flex items-center gap-2">
        <Button variant="outline" size="icon" className={cn("h-11 w-11", isRefreshing && "animate-spin")} onClick={onRefresh}>
          <RefreshCw className="h-5 w-5" />
        </Button>
        {showAdminActions && (
          <Button variant="outline" size="icon" className="h-11 w-11 border-primary/20 text-primary" onClick={onOpenLocationMgr}>
        ...
        <div className="flex bg-muted p-1 rounded-lg border ml-2">
          ...
          <Button variant={viewMode === "map" ? "secondary" : "ghost"} size="sm" onClick={() => setViewMode("map")}><Building2 className="h-4 w-4" /></Button>
        </div>
      </div>
    </div>
  );
