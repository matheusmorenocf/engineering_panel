    ...
    <div className="p-6 space-y-6 flex flex-col h-screen bg-background">
      ...
      <div className="flex gap-4 items-center bg-card p-4 rounded-xl border shadow-sm shrink-0">
        <div className="relative flex-1">
          ...
          <Input placeholder="Buscar..." className="pl-10 h-11 bg-background" value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
                  ...
                  {products.map((p: any) => (
                    <Card key={p.id} className="bg-card border border-border shadow-sm hover:shadow-md transition-all cursor-pointer group" onClick={() => setSelectedItem(p)}>
                      <div className="aspect-video bg-muted relative overflow-hidden">
                        <div className="absolute inset-0 flex items-center justify-center text-muted-foreground/20 italic"><ImageIcon className="h-10 w-10" /></div>
                      ...
                      <CardContent className="p-4">
                        ...
                        <div className="mt-2 text-[9px] font-black uppercase p-1.5 rounded bg-warning/10 text-warning border border-warning/20 text-center">{p.tracking_code}</div>
                      </CardContent>
                    </Card>
                  ))}
                </div>
              ...
              <Card key={group.nf} className="bg-card border border-border shadow-md overflow-hidden">
                <div className="bg-muted/50 p-4 border-b flex justify-between items-center">
                  <div className="flex items-center gap-4">
                    ...
                    <div><h2 className="text-lg font-black uppercase tracking-tight">{group.nf}</h2><p className="text-[10px] text-muted-foreground font-black uppercase">Origem: {group.sender || "N/A"}</p></div>
                  </div>
                </div>
                <Table>
                  <TableBody>
                    {group.products.map((product: any) => (
                      <TableRow key={product.id} className="hover:bg-muted/30 cursor-pointer border-border" onClick={() => setSelectedItem(product)}>
                        <TableCell className="w-[80px]"><div className="h-12 w-12 rounded-lg bg-muted flex items-center justify-center"><ImageIcon className="h-6 w-6 text-muted-foreground/20" /></div></TableCell>
                        <TableCell><p className="font-black text-sm uppercase line-clamp-1">{product.product}</p><p className="text-[10px] text-muted-foreground font-bold font-mono">ID: {product.tracking_code}</p></TableCell>
                        <TableCell><Badge variant="outline" className="text-primary border-primary/20 bg-primary/5 font-black uppercase text-[10px]"><MapPin className="h-3 w-3 mr-1" /> {product.location}</Badge></TableCell>
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto z-[9999]">
          <DialogHeader><DialogTitle className="uppercase font-black tracking-tight">Registrar Recebimento</DialogTitle></DialogHeader>
